...
const targetDate = new Date("2026-09-19T08:30:00").getTime();
...
onSnapshot(rsvpRef, (snapshot) => {
    ...
    snapshot.forEach((doc) => {
        ...
        if (data.kehadiran === "Hadir") {
            totalTetamu += Number(data.jumlahTetamu || 0);
        }
    });

    console.log("RSVP Snapshot:", snapshot.size, totalTetamu);

    updateRSVPStat();

});

function updateRSVPStat() {

    const responCount = document.getElementById("responCount");
    const jumlahTetamuCount = document.getElementById("jumlahTetamuCount");

    if (responCount) {
        responCount.textContent = totalRespon;
    }

    if (jumlahTetamuCount) {
        jumlahTetamuCount.textContent = totalTetamu;
    }
